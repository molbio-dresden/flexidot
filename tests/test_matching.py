...
def test_basic_functionality_protein():
    ...
    seq2 = Seq("KVLYM")
    wordsize = 3
    x1, y1, x2, y2 = find_match_pos_diag(seq1, seq2, wordsize, type_nuc=False, rc_option=False)
    assert len(x1) > 0
    assert len(y1) > 0
    assert len(x2) == 0
    assert len(y2) == 0
...
def test_ambiguous_residues_no_conversion():
    ...
    x1, y1, x2, y2 = find_match_pos_diag(seq1, seq2, wordsize, convert_wobbles=False, max_N_percentage=60)
    assert len(x1) == 0
    assert len(y1) == 0
    assert len(x2) == 0
    assert len(y2) == 0
...
def test_ambiguous_residues_with_conversion():
    ...
    seq2 = Seq("NNNNGATC")
    wordsize = 4
    x1, y1, x2, y2 = find_match_pos_diag(seq1, seq2, wordsize, convert_wobbles=True, max_N_percentage=26)
...
def test_report_lcs_nucleotide():
    ...
    seq2 = Seq("GATCGATC")
    wordsize = 4
    x1, y1, x2, y2, lcs_for, lcs_rev = find_match_pos_diag(seq1, seq2, wordsize, report_lcs=True)
    assert lcs_for == 7
    assert lcs_rev == 7
...
def test_report_lcs_protein():
    ...
    seq2 = Seq("KVLYM")
    wordsize = 3
    x1, y1, x2, y2, lcs_for, lcs_rev = find_match_pos_diag(seq1, seq2, wordsize, report_lcs=True, type_nuc=False)
    assert lcs_for > 0
    assert lcs_rev == 0
