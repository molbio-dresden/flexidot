...
def load_modules():
    ...
    global cllct, gridspec, patches, rcParams, mplrc, P, Color, SeqIO, np, mcolors, rgb2hex, regex, PdfPages
    ...
    try:
        ...
        command = module_install_command("tkinter", upgrade=True)
        try:
            os.system(command)
            print("\n")
            import pylab as P
            ...
            print("Please install module matplotlib manually")
            print("\n>>> In case of error with 'tkinter' under linux: try the following command:")
...
def usage():
    ...
    -i, --in_file                  Input fasta file (fasta file name or comma-separated file list)
                                   > To provide multiple files, recall -i or provide comma-separated file names
    ...
    -p, --plotting_mode            Mode of FlexiDot dotplotting
                                   ...
                                   > To run multiple plotting modes, recall -p or provide comma-separated numbers
    ...
    -N, --narrow_diagonal_interval  Interval size [integer] for quicker narrow self-dotplot - limits calculation to
                                    interval of given size along the main diagonal (only if --plotting_mode=0)
                                    [inactive by default]
    ...
    GFF SHADING (not for -p/--plotting_mode=1)...

    -g, --input_gff_files          GFF3 file used for markup in self-dotplots and poly-dotplots
                                   > To provide multiple files, recall -g or provide comma-separated file names)

    -G, --gff_color_config_file    Tab-delimited config file for custom GFF-based shading
...
def check_input(argv, trial_mode=False):
    ...
    arguments = ["-a", "--auto_fas",                 "a", "auto_fas",
                 ...
                 "-N", "--narrow_diagonal_interval", "N:", "narrow_diagonal_interval=",
    ...
    narrow_diagonal_interval  = False
    ...
    for opt, arg in opts:
            ...
            if "," in arg:
                ...
                for temp_file in arg_list:
                    if not os.path.exists(str(temp_file)):
                        ...
                        print("fasta file #%d: %s" % (len(input_fasta), str(temp_file)))
                        log_txt += "\nfasta file #%d: %s" % (len(input_fasta), str(temp_file))
            else:
                if not os.path.exists(str(arg)):
                    ...
                    print("fasta file #%d: %s" % (len(input_fasta), str(arg)))
                    log_txt += "\nfasta file #%d: %s" % (len(input_fasta), str(arg))
        ...
        # multiple gff files: reads them into a list
            ...
            if "," in arg:
                ...
                for temp_file in arg_list:
                    if not os.path.exists(str(temp_file)):
                        ...
                        print("GFF file #%d: %s" % (len(input_gff_files), str(temp_file)))
                        log_txt += "\nGFF file #%d: %s" % (len(input_gff_files), str(temp_file))
                ...
                if not os.path.exists(str(arg)):
                    ...
                    print("GFF file #%d: %s" % (len(input_gff_files), str(arg)))
                    log_txt += "\nGFF file #%d: %s" % (len(input_gff_files), str(arg))
            ...
            if not os.path.exists(str(arg)):
                ...
                message += "\n  -->Running FlexiDot with default input_user_matrix_file! Invalid file %s" % (str(arg))
                print(message)
                log_txt += message
            else:
            ...
            try: m_col              = int(arg)
                ...
                print("Invalid argument for m_col - using default value")
                log_txt += "\nInvalid argument for m_col - using default value"

        elif opt in ("-n", "--n_row"):
            try: n_row              = int(arg)
            except:
                print("Invalid argument for n_row - using default value")
                log_txt += "\nInvalid argument for n_row - using default value"

        elif opt in ("-f", "--filetype"):
            if arg.isdigit() and 0 <= int(arg) <= 2:
                filetype    = int(arg)
            elif arg.lower() in ("pdf", "png", "jpg"):
                filetype = arg.lower()
            else:
            ...
            try: wordsize           = int(arg)
                ...
                print("Invalid argument for wordsize - using default value")
                log_txt += "\nInvalid argument for wordsize -  default value"
            ...
            try: substitution_count         = int(arg)
                ...
                print("Invalid argument for substitution_count - using default value")
                log_txt += "\nInvalid argument for substitution_count - using default value"
        ...
        elif opt in ("-N", "--narrow_diagonal_interval"):
            if str(arg).isdigit():
                narrow_diagonal_interval
                try:
                    narrow_diagonal_interval = int(arg)
                except:
                    log_txt += "\nInvalid argument for narrow_diagonal_interval - set as False [default]"
                    print("\nInvalid argument for narrow_diagonal_interval - set as False [default]." % (lcs_shading_ref))
            else:
                log_txt += "\nInvalid argument for narrow_diagonal_interval - set as False [default]"
                print("\nInvalid argument for narrow_diagonal_interval - set as False [default]." % (lcs_shading_ref))
            ...
            try: lcs_shading_num    = int(arg) - 1
                ...
                print("Invalid argument for lcs_shading_num - using default value")
                log_txt += "\nInvalid argument for lcs_shading_num - using default value"
            ...
            try:
                ...
                print("Invalid argument for lcs_shading_ref - using default value")
                log_txt += "\nInvalid argument for lcs_shading_ref - using default value"

        elif opt in ("-Y", "--lcs_shading_interval_len"):
            try: lcs_shading_interval_len = int(arg)
            except:
                print("Invalid argument for lcs_shading_interval_len - using default value")
                log_txt += "\nInvalid argument for lcs_shading_interval_len - using default value"
            ...
            try: plot_size         = float(arg)
                ...
                print("Invalid argument for plot_size - using default value")
                log_txt += "\nInvalid argument for plot_size - using default value"

        elif opt in ("-A", "--line_width"):
            try: line_width         = float(arg)
            except:
                print("Invalid argument for line_width - using default value")
                log_txt += "\nInvalid argument for line_width - using default value"

        elif opt in ("-B", "--line_col_for"):
            if mcolors.is_color_like(arg):
                line_col_for       = arg
            else:
                print("Invalid argument for line_col_for - using default value")
                log_txt += "\nInvalid argument for line_col_for - using default value"

        elif opt in ("-C", "--line_col_rev"):
            if mcolors.is_color_like(arg):
                line_col_rev       = arg
            else:
                print("Invalid argument for line_col_rev - using default value")
                log_txt += "\nInvalid argument for line_col_rev - using default value"
            ...
            try: label_size         = float(arg)
                ...
                print("Invalid argument for label_size - using default value")
                log_txt += "\nInvalid argument for label_size - using default value"

        elif opt in ("-F", "--spacing"):
            try: spacing            = float(arg)
            except:
                print("Invalid argument for spacing - using default value")
                log_txt += "\nInvalid argument for spacing - using default value"
            ...
            try: title_length     = int(arg)
                ...
                try:
                    ...
                    if arg[-1].upper() in ["B", "E"]: # B (beginning), E (end)
                        ...
                        print("Invalid title_length position information - using default value")
                        log_txt += "\nInvalid title_length position information - using default value"
                except:
                    print("Invalid argument for title_length - using default value")
                    log_txt += "\nInvalid argument for title_length - using default value"
    ...
    if 0 in plotting_modes or 1 in plotting_modes:
    ...
    if len(input_gff_files) != 0 and (0 in plotting_modes or 2 in plotting_modes):
        text += "\n" + "Input gff files:                                   " + ", ".join(input_gff_files)
        if gff_color_config_file != "":
            text += "\n" + "GFF color config file:                             " + gff_color_config_file

    if 0 in plotting_modes and narrow_diagonal_interval != False:
        text += "\n" + "Narrowed window for fast self-dotplots:            " + str(narrow_diagonal_interval)
    ...
    if start:
        ...
        if prefix != "" and prefix != None:
            if prefix[-1] not in ["-", "_"]:
                prefix = prefix + "_"
            log_file_name = prefix + log_file_name.lower()
...
def read_gff_color_config(gff_color_config_file=""):
    ...
    text = "GFF color specification updated acc. to %s\n\t%s\n\n" % (gff_color_config_file, ", ".join(sorted(gff_feat_colors.keys())))
    logprint(text, start=False, printing=True)
...
def read_matrix(matrix_file_name, delim="\t", symmetric=True, recursion=False, verbose=False):
    ...
    if len(names) == 0 and not recursion:
        if delim == "\t":
            new_delim = ","
        else:
            new_delim = "\t"
...
def legend_figure(colors, lcs_shading_num, type_nuc=True, unit="%", filetype="png", max_len=None, min_len=0, bins=[], alphas=[], gff_legend=False, prefix="", verbose=False):
    ...
    if not gff_legend and ((len(bins) != 0 and len(colors) != lcs_shading_num+1) or (len(bins) != 0 and len(colors) != len(bins)+1)):
...
def find_match_pos_diag(seq1, seq2, wordsize, rc_option=True, narrow_diagonal_interval=False, report_lcs=False, convert_wobbles=False, max_N_percentage=49, type_nuc=True, verbose=False):
    """
    ...
    if narrow_diagonal_interval, only consider diagonal lines within a certain distance (narrow_diagonal_interval) to the middle diagonal
    """
    global t1 # timer
    ...
    if verbose:
        if rc_option:
            text = "[matches: %d for; %d rc]" % (len(matches_for), len(matches_rc))
        else:
            text = "[matches: %d for; no rc]" % (len(matches_for))
        logprint(text, start=False, printing=True)

    if narrow_diagonal_interval not in [None, "", 0, False]:
        rc_threshold = narrow_diagonal_interval // 2 + (narrow_diagonal_interval % 2)

    # create lists of x and y co-ordinates for scatter plot
    # keep all coordinates of all shared kmers (may match multiple times)
    diag_dict_for = {}
    diag_dict_rc  = {}
    for (match_list, pos_dict1, pos_dict2, diag_dict, diag_ori) in [(matches_for, kmer_pos_dict_one, kmer_pos_dict_two, diag_dict_for, "for"),
                                                                    (matches_rc, kmer_pos_dict_three, kmer_pos_dict_four, diag_dict_rc, "rc")]:
        for kmer in match_list:
            for i in pos_dict1[kmer]:
                for j in pos_dict2[kmer]:
                    diag = i-j
                    # no narrow window or forward within narrow window
                    if narrow_diagonal_interval in [None, "", 0, False] or (diag_ori == "for" and (abs(diag) <= narrow_diagonal_interval)):
                        lower_bound = i+1
                        upper_bound = i+1+wordsize
                        update_dict = True
                    # recalculate values for reverse if overlapping window
                    elif diag_ori == "rc":
                        lower_bound = i+1
                        upper_bound = i+1+wordsize

                        min_val = int((len_two + diag)//2 + (len_two + diag)%2 - rc_threshold + 1)
                        max_val = int((len_two + diag)//2 + rc_threshold + 1)

                        # check, if lower bounds within narrowed window
                        check1 = min_val <= lower_bound <= max_val
                        check2 = min_val <= upper_bound <= max_val

                        # both bounds within window
                        if check1 and check2:
                            update_dict = True
                        # lower_bound in window - upper_bound not
                        elif check1:
                            upper_bound = max_val
                            update_dict = True
                        # upper_bound in window - lower_bound not
                        elif check2:
                            lower_bound = min_val
                            update_dict = True
                        else:
                    ...
                    if update_dict:
                        points = set(range(lower_bound, upper_bound))
                        if len(points) != 0:
    ...
    x1 = [] # x values reverse
    y1 = [] # y values forward
    for diag in sorted(diag_dict_for.keys()):
    ...
    y2 = [] # y values rc
    if rc_option:
        for diag in sorted(diag_dict_rc.keys()):
...
def find_match_pos_regex(seq1, seq2, wordsize, rc_option=True, narrow_diagonal_interval=False, report_lcs=False, convert_wobbles=False, max_N_percentage=49, type_nuc=True, verbose=False, substitution_count=0):
    """
    ...
    ambiq_residues = "[%s]" % ("".join(sorted(general_ambiguity_code.keys())))
    ...
    if rc_option:
        data_list = [(str(seq_one), str(seq_two), diag_dict_for, 0, "for"),
                     (str(seq_one), str(seq_two.reverse_complement()), diag_dict_rc, 1, "rc")]
    else:
        data_list = [(str(seq_one), str(seq_two), diag_dict_for, 0, "for")]
    ...
    if verbose:
        if rc_option:
            text = "[matches: %d for; %d rc]" % (counter[0], counter[1])
        else:
            text = "[matches: %d for; no rc]" % (counter[0])
        logprint(text, start=False, printing=True)
...
def selfdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, rc_option=True, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, narrow_diagonal_interval=False):
    """
    ...
    if gff_files != None and len(gff_files) != 0:
        legend_prefix = prefix + "Self"
    ...
    print("\n%s\n\nCreating %s selfdotplot images\n%s\n\n=>" % (60*"=", len(sequences), 40*"-"), end=" ")
    log_txt = "\n%s\n\nCreating %s selfdotplot images\n%s\n\n=>" % (60*"=", len(sequences), 40*"-")

    # preparations for file name
    if not narrow_diagonal_interval:
        name_graph = "Selfdotplot"
    else:
        name_graph = "NarrowSelfdotplot"
    suffix = ""
    ...
    if filetype == "pdf":
        fig_name = "%s%s_ws%d%s.%s" % (prefix, name_graph, wordsize, suffix, filetype)
    ...
    for seq_name in sequences:
        ...
        if substitution_count != 0:
            # print("RE")
            x_lists, y_lists, x_lists_rc, y_lists_rc = find_match_pos_regex(seq_one, seq_one, wordsize, rc_option=rc_option, substitution_count=substitution_count, convert_wobbles=convert_wobbles, narrow_diagonal_interval=narrow_diagonal_interval, max_N_percentage=max_N_percentage, type_nuc=type_nuc, verbose=verbose)
        else:
            # print("DIAG")
            x_lists, y_lists, x_lists_rc, y_lists_rc = find_match_pos_diag(seq_one, seq_one, wordsize, rc_option=rc_option, convert_wobbles=convert_wobbles, narrow_diagonal_interval=narrow_diagonal_interval, max_N_percentage=max_N_percentage, type_nuc=type_nuc, verbose=verbose)
            ...
            if col != "white" and len(x_lines) != 0:
                for ldx in range(len(x_lines)):
                    try:
                        ...
                        print("Proceeding after error with line collection at index %d (data=%s) in Selfdotplot" % (ldx, str(x_lines[ldx])))
        ...
        if multi:
            ...
            if counter == ncols * nrows:
                ...
                # save figure
                if filetype == "pdf":
                    P.suptitle("Page %.3d" % (page_counter), size=label_size*1.5, weight="bold", y=1.02)
                    pdf.savefig(bbox_inches='tight')
                else:
                    # name and create output files (names derived from SEQNAME)
                    fig_name = "%s%s_ws%d%s-%.3d.%s" % (prefix, name_graph, wordsize, suffix, page_counter, filetype)
                    list_of_fig_names.append(fig_name)
        ...
        # plotting separate figure files
            ...
            # save figure
            if filetype == "pdf":
                P.suptitle("Page %.3d" % (counter), size=label_size*1.5, weight="bold", y=1.02)
                pdf.savefig(bbox_inches='tight')
            else:
                # name and create output files (names derived from SEQNAME)
                fig_name = "%s%s-%d_%s_ws%d%s.%s" % (prefix, name_graph, counter, shorten_name(name_seq, max_len=title_length, title_clip_pos=title_clip_pos), wordsize, suffix, filetype)
                list_of_fig_names.append(fig_name)
                P.savefig(fig_name, bbox_inches='tight')
...
def pairdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, rc_option=True, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, x_label_pos_top=True, only_vs_first_seq=False, length_scaling=True, scale_delim_col="red"):
    ...
    if len(sequences) < 2:
        text = "\n%s\n\nCreating %d paired dotplot image \n%s\n\n=>" % (60*"=", len(sequences)*(len(sequences)-1)//2, 40*"-")
    ...
    lcs_data_file = open("%sPairdotplot_LCS_data_file_ws%d%s.txt" % (prefix, wordsize, suffix.replace("_scaled", "").replace("_collage", "")), 'w')
    ...
    for idx in range(len(sequences)-1):
        ...
        for jdx in range(idx+1, len(sequences)):
            ...
            if substitution_count != 0:
                # print("RE")
                x1, y1, x2, y2, lcs_for, lcs_rev = find_match_pos_regex(seq_one, seq_two, wordsize, rc_option=rc_option, substitution_count=substitution_count, convert_wobbles=convert_wobbles, max_N_percentage=max_N_percentage, report_lcs=True, type_nuc=type_nuc, verbose=verbose)
            else:
                # print("DIAG")
                x1, y1, x2, y2, lcs_for, lcs_rev = find_match_pos_diag(seq_one, seq_two, wordsize, rc_option=rc_option, convert_wobbles=convert_wobbles, max_N_percentage=max_N_percentage, report_lcs=True, type_nuc=type_nuc, verbose=verbose)

            # prevent ZeroDivisionError in case of empty sequences
            try:
                lcs_for_percentage = round(lcs_for*100./min(len_one, len_two), 3)
                lcs_rev_percentage = round(lcs_rev*100./min(len_one, len_two), 3)
            except:
                lcs_for_percentage = "NA"
                lcs_rev_percentage = "NA"

            # write LCS data file
            lcs_data_file.write("\t".join([name_one, name_two, str(len_one), str(len_two),
                                           str(lcs_for), str(lcs_for_percentage),
                                           str(lcs_rev), str(lcs_rev_percentage)]) + "\n")
                ...
                if col != "white" and len(x_lines) != 0:
                    for ldx in range(len(x_lines)):
                        try:
                            ...
                            print("Proceeding after error with line collection at index %d (data=%s) in Pairdotplot" % (ldx, str(x_lines[ldx])))
            color_list = np.array(color_list)
            ...
            # format axes
            P.xlabel(unicode_name("%s [%s]" % (shorten_name(name_one, max_len=title_length, title_clip_pos=title_clip_pos), aa_bp_unit)), fontsize=label_size, fontweight='bold', labelpad=4)
            P.ylabel(unicode_name("%s [%s]" % (shorten_name(name_two, max_len=title_length, title_clip_pos=title_clip_pos), aa_bp_unit)), fontsize=label_size, fontweight='bold', labelpad=4)
            P.tick_params(axis='both', which='major', labelsize=label_size*.9)
            ...
            if multi and counter == ncols * nrows:
                ...
                # save figure
                if filetype == "pdf":
                    P.suptitle("Page %.3d" % (page_counter), size=label_size*1.5, weight="bold", y=page_num_pos)
                    pdf.savefig(bbox_inches='tight')
                else:
            ...
            # plotting separate figure files
                ...
                # save figure
                if filetype == "pdf":
                    P.suptitle("Page %.3d" % (counter), size=label_size*1.5, weight="bold", y=page_num_pos)
                    pdf.savefig(bbox_inches='tight')
                else:
                    # name and create output files
                    fig_name = "%s%s-%d_ws%d%s.%s" % (prefix, name_graph, counter, wordsize, suffix, filetype)
                    list_of_fig_names.append(fig_name)
                    P.savefig(fig_name)

                # # check currently open figures
                # print ([manager.canvas.figure for manager in matplotlib._pylab_helpers.Gcf.get_all_fig_managers()])
                P.close()
                P.cla()
...
def polydotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, rc_option=True, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, x_label_pos_top=True, lcs_shading=True, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, lcs_shading_num=5, spacing=0.04, input_user_matrix_file="", user_matrix_print=True, rotate_labels=False):
    ...
    if len(sequences) == 0:
        text = "\n%s\n\nCreating %dx%d polydotplot image\n%s\n\n=>" % (60*"=", len(sequences), len(sequences), 40*"-")
    ...
    if gff_files != None and len(gff_files) != 0 or lcs_shading:
        legend_prefix = prefix + "Poly"
    ...
    if input_user_matrix_file != "" and input_user_matrix_file != None:
        ...
        if 0 <= custom_min < 1 and 0 < custom_max <= 1:
            ...
            custom_min = max(0, (multi_factor*custom_min // rounding_factor) * (rounding_factor*1./multi_factor))
            custom_max = min((multi_factor*custom_max // rounding_factor) * (rounding_factor*1./multi_factor), 1)
            text += "new (%.2f, >%2f)\n" % (custom_min, custom_max)
    ...
    if substitution_count != 0:
        lcs_data_file = open("%sPolydotplot_LCS_data_file_ws%d_S%d%s.txt" % (prefix, wordsize, substitution_count, suffix), 'w')
    else:
        lcs_data_file = open("%sPolydotplot_LCS_data_file_ws%d%s.txt" % (prefix, wordsize, suffix), 'w')
    lcs_data_file.write("\t".join(["#title1", "title2", "len_seq1", "len_seq2", "len_lcs_for", "%_min_seq_len", "len_lcs_rev", "%_min_seq_len"])+"\n")
    ...
    if verbose:
        logprint("\n\nlcs_dict\n" + str(sorted(lcs_dict.items())))
        if custom_shading:
            logprint("\ncustom_dict\n" + str(sorted(custom_dict.items())))
        ...
        for jdx in range(idx, len(sequences)):
            ...
            for kdx in range(len(counters)):
                ...
                if representation == 0 or len(counters) == 1 or (representation == 1 and kdx == 0) or (representation == 2 and kdx == 1):
                    ...
                    if idx == jdx and gff_files != None and len(gff_files) != 0:
                            ...
                            if col != "white" and len(x_lines) != 0:
                                for ldx in range(len(x_lines)):
                                    try:
                                        ...
                                        print("Proceeding after error with line collection at index %d (data=%s) in Polydotplot" % (ldx, str(x_lines[ldx])))
                        color_list = np.array(color_list)
    ...
    if lcs_shading:
        if lcs_shading_ref == 1: # percentage of shorter sequence
            legend_file_name = legend_figure(colors, lcs_shading_num, unit="%", filetype=filetype, prefix=legend_prefix)
        elif lcs_shading_ref == 2: # interval sizes
            legend_file_name = legend_figure(colors, lcs_shading_num, unit=aa_bp_unit, filetype=filetype, prefix=legend_prefix, bins=color_bins)
        else: # relative of maximum lcs
            legend_file_name = legend_figure(colors, lcs_shading_num, unit=aa_bp_unit, filetype=filetype, prefix=legend_prefix, max_len=max_lcs)

    if custom_shading:
        custom_prefix = "_custom-matrix_"
        legend_file_name_custom = legend_figure(colors_2, lcs_shading_num, unit="%", filetype=filetype, prefix=legend_prefix+custom_prefix, max_len=custom_max, min_len=custom_min)
...
def main(seq_list, wordsize, modes=[0, 1, 2], prefix=None, plot_size=10, label_size=10, filetype="png", type_nuc=True, convert_wobbles=False, substitution_count=0, rc_option=True, alphabetic_sorting=False, only_vs_first_seq=False, gff=None, multi=True, ncols=1, nrows=1, narrow_diagonal_interval=False, lcs_shading=True, lcs_shading_num=5, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, gff_color_config_file="", input_user_matrix_file="", user_matrix_print=False, length_scaling=True, title_length=50, title_clip_pos="B", spacing=0.04, max_N_percentage=49, mirror_y_axis=False, verbose=False):
    ...
    if input_gff_files != None and len(input_gff_files) != 0:
        if gff_color_config_file not in ["", None]:
            text = "\n%s\n\nReading GFF color configuration file\n%s\n\n=> %s\n" % (60*"=", 40*"-", gff_color_config_file)
            logprint(text, start=False, printing=True)
    ...
    text = "%s\n\nRunning plotting modes %s" % (60*"=", ", ".join(mode_text))
    logprint(text, start=False, printing=True)
    ...
    t1 = time.time()
    if 0 in modes:
        list_of_fig_names = selfdotplot(seq_list, wordsize, prefix=prefix, label_size=label_size, title_length=title_length, title_clip_pos=title_clip_pos, plot_size=plot_size, filetype=filetype, type_nuc=type_nuc, convert_wobbles=convert_wobbles, rc_option=rc_option, substitution_count=substitution_count, alphabetic_sorting=alphabetic_sorting, multi=multi, ncols=ncols, nrows=nrows, gff_files=gff, gff_color_dict=gff_feat_colors, mirror_y_axis=mirror_y_axis, max_N_percentage=max_N_percentage, verbose=verbose, narrow_diagonal_interval=narrow_diagonal_interval)
        t1 = time_track(t1)
...
if trial_mode:
    input_fasta        = ["Test.fas"]
    output_file_prefix = "#Test_"
    # input_gff_files    = ["Test.gff3"]
    # input_user_matrix_file = "Test_matrix.txt"
