...
def usage():
    ...
    Citation: 
    Kathrin M. Seibt, Thomas Schmidt, Tony Heitkam (in prep.) 
    "FlexiDot: Highly customizable ambiguity-aware dotplots for visual sequence investigation"
    ...
    -T, --title_length             Limit title length for self dotplot comparison
                                   Use last characters of the title name (instead of the first): add an "E" (end), e.g. -T 20E
                                    [default = 20, the first 20 characters]
...
def check_input(argv, trial_mode=False):
    ...
    title_selectpos        = "B" # B (begin), E (end)
    ...
    text += "\n" + "Spacing:                                           " + str(spacing)
    text += "\n" + "Title length (limit number of characters):         " + str(title_length)
    if title_selectpos == "E": 
        text += "the last %i characters are printed." %int(title_length)
    else:
        text += "the first %i characters are printed." %int(title_length)
    ...
    # collect settings
    parameters = [commandline, auto_fas, input_fasta, output_file_prefix, collage_output, m_col, n_row, filetype_dict[filetype], type_nuc, input_gff_files, gff_color_config_file, wordsize, plotting_modes, wobble_conversion, substitution_count, rc_option, alphabetic_sorting, lcs_shading, lcs_shading_num, lcs_shading_ref, lcs_shading_interval_len, lcs_shading_ori, input_user_matrix_file, user_matrix_print, plot_size, line_width, line_col_for, line_col_rev, x_label_pos, label_size, spacing, length_scaling, title_length, title_selectpos, verbose]
...
def shorten_name(seq_name, max_len=20, title_selectpos="B"): #, delim="_"):
    """
...
def legend_figure(colors, lcs_shading_num, type_nuc=True, unit="%", filetype="png", max_lcs_len=None, min_lcs_len=0, bins=[], alphas=[], gff_legend=False, prefix="", verbose=False):
    ...
    if gff_legend:
        label_text = bins[:]
        edge_col = None
        legend_file_name = "Selfdotplot_GFF_Shading_Legend_n%d." % lcs_shading_num + filetype
...
def selfdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, title_length=float("Inf"), title_selectpos="B"):
    """
    ...
    if gff_files != None and gff_files != []:
        ...
        logprint(text, start=False, printing=True)
        feat_dict = read_gffs(gff_files, color_dict=gff_color_dict, type_nuc=type_nuc, prefix=prefix, filetype=filetype, verbose=verbose)

    # check input variables
    if convert_wobbles and max_N_percentage > 49:
        max_N_percentage=49
        text = "Provide valid max_N_percentage, kmers with >=50% Ns are ignored\n"
        logprint(text, start=False, printing=True)
    ...
    for seq_name in sequences:
        ...
        if multi:
            ...
            P.title(unicode_name(shorten_name(name_seq, max_len=title_length, title_selectpos=title_selectpos)), fontsize=label_size, fontweight='bold')
        ...
        # plotting separate figure files
            ...
            P.ylabel("[%s]" % aa_bp_unit, fontsize=label_size)
            P.tick_params(axis='both', which='major', labelsize=label_size*.9)
            P.title(unicode_name(shorten_name(name_seq, max_len=title_length, title_selectpos=title_selectpos)), fontsize=label_size*1.3, fontweight='bold')

            # name and create output files (names derived from SEQNAME)
            fig_name = '%s%s-%d_%s_wordsize%i%s.%s' %(prefix, name_graph, counter, name_seq, wordsize, suffix, filetype)
            P.savefig(fig_name, bbox_inches='tight')
...
def pairdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, length_scaling=True, scale_delim_col="red", title_length=float("Inf"), title_selectpos="B"):
    ...
    if convert_wobbles and max_N_percentage > 49:
        max_N_percentage=49
        text = "Provide valid max_N_percentage, kmers with >50% are ignored\n"
        logprint(text, start=False, printing=True)
    ...
    for idx in range(len(sequences)-1):
        ...
        for jdx in range(idx+1, len(sequences)):
            ...
            # format axes
            P.xlabel(unicode_name(shorten_name(name_one, max_len=title_length, title_selectpos=title_selectpos)) + " [%s]" % aa_bp_unit, fontsize=label_size, fontweight='bold', labelpad=4)
            P.ylabel(unicode_name(shorten_name(name_two, max_len=title_length, title_selectpos=title_selectpos)) + " [%s]" % aa_bp_unit, fontsize=label_size, fontweight='bold', labelpad=4)
            P.tick_params(axis='both', which='major', labelsize=label_size*.9)
    ...
    for idx in range(len(sequences)):
        ...
        for jdx in range(idx, len(sequences)):
            ...
            if idx == jdx:
                ...
                counter = counter + (counter - 1) // (nrows) # + row_pos
                counters = [counter]
            ...
            for kdx in range(len(counters)):
                ...
                if x_label_bool: # x title and labels on top or bottom
                    P.xlabel(unicode_name(shorten_name(n1, max_len=title_length, title_selectpos=title_selectpos)), fontsize=label_size, rotation=x_label_rotation, verticalalignment=xvalign, horizontalalignment=xhalign, fontweight='bold', labelpad=8) # axis naming
                ...
                if fig_pos % ncols == 1 or (ncols == 1 and nrows == 1): # y title and labels in 1st column
                    P.ylabel(unicode_name(shorten_name(n2, max_len=title_length, title_selectpos=title_selectpos)), fontsize=label_size, rotation=y_label_rotation, verticalalignment=yvalign, horizontalalignment=yhalign, fontweight='bold', labelpad=8)
...
def main(seq_list, wordsize, modes=[0, 1, 2], prefix=None, plot_size=10, label_size=10, filetype="png", type_nuc=True, convert_wobbles=False, substitution_count=0, rc_option=True, alphabetic_sorting=False, gff=None, multi=True, ncols=1, nrows=1, lcs_shading=True, lcs_shading_num=5, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, gff_color_config_file="", input_user_matrix_file="", user_matrix_print=False, length_scaling=True, title_length=50, title_selectpos="B", spacing=0.04, verbose=False):
    ...
    t1 = time.time()
    if 0 in modes:
        list_of_png_names = selfdotplot(seq_list, wordsize, prefix=prefix, label_size=label_size, title_length=title_length, title_selectpos=title_selectpos, plot_size=plot_size, filetype=filetype, type_nuc=type_nuc, convert_wobbles=convert_wobbles, substitution_count=substitution_count, alphabetic_sorting=alphabetic_sorting, multi=multi, ncols=ncols, nrows=nrows, gff_files=gff, gff_color_dict=gff_feat_colors, verbose=verbose)
        t1 = time_track(t1)
    ...
    # all-against-all dotplot
    if 2 in modes:
        list_of_png_names = polydotplot(seq_list, wordsize, prefix=prefix, label_size=label_size, title_length=title_length, title_selectpos=title_selectpos, plot_size=plot_size, filetype=filetype, type_nuc=type_nuc, convert_wobbles=convert_wobbles, substitution_count=substitution_count, alphabetic_sorting=alphabetic_sorting, lcs_shading=lcs_shading, lcs_shading_num=lcs_shading_num, lcs_shading_ref=lcs_shading_ref, lcs_shading_interval_len=lcs_shading_interval_len, lcs_shading_ori=lcs_shading_ori, input_user_matrix_file=input_user_matrix_file, user_matrix_print=user_matrix_print, spacing=spacing, verbose=verbose)
        t1 = time_track(t1)
        if list_of_png_names != [] and list_of_png_names != None:
            text = "-> Image file(s): %s\n" % ", ".join(list_of_png_names)
        else:
...
# read out parameters
commandline, auto_fas, input_fasta, output_file_prefix, collage_output, m_col, n_row, filetype, type_nuc, input_gff_files, gff_color_config_file, wordsize, plotting_modes, wobble_conversion, substitution_count, rc_option, alphabetic_sorting, lcs_shading, lcs_shading_num, lcs_shading_ref, lcs_shading_interval_len, lcs_shading_ori, input_user_matrix_file, user_matrix_print, plot_size, line_width, line_col_for, line_col_rev, x_label_pos_top, label_size, spacing, length_scaling, title_length, title_selectpos, verbose = parameters

# evtl. overwrite parameters for testing purposes in trial mode
if trial_mode:
    # input_user_matrix_file = "AngioSINE-v18-alignment-identities.csv"
    input_fasta        = ["test-sequences-9-Ns.fas"]
    input_fasta        = ["Beta_SINEs__select_consensus.fas"]
    # input_user_matrix_file = "Beta_SINEs__select_consensus_matrix.txt"
    # input_user_matrix_file = "Beta_SINEs__select_consensus_matrix-01.txt"
    # input_user_matrix_file = "Beta_SINEs__select_consensus_matrix-comma-str.txt"
    # input_user_matrix_file = "Beta_SINEs__select_consensus_matrix-100+.txt"
    # user_matrix_print  = True
    output_file_prefix = "SINEmatrix"
    output_file_prefix = "SINEmatrix-NoShading"
    plot_size = 10
