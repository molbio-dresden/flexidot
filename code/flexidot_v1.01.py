...
def usage():
    ...
    -T, --title_length             Limit title length for self dotplot comparison (only if --plotting_mode=0)
                                    [default = infinite]  
...
def check_input(argv, trial_mode=False):
    ...
    for opt, arg in opts:
        ...
        if opt in ("-v", "--verbose"):
            ...
            auto_fas           = check_bools(str(arg), default=auto_fas)
...
def shorten_name(seq_name, max_len=float("Inf"), delim="_"):
    """
...
def selfdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, title_length=float("Inf")):
    """
    ...
    for seq_name in sequences:
        ...
        if multi:
            ...
            P.title(unicode_name(shorten_name(name_seq, max_len=title_length)), fontsize=label_size, fontweight='bold')
        ...
        # plotting separate figure files
            ...
            P.ylabel("[%s]" % aa_bp_unit, fontsize=label_size)
            P.tick_params(axis='both', which='major', labelsize=label_size*.9)
            P.title(unicode_name(shorten_name(name_seq, max_len=title_length)), fontsize=label_size*1.3, fontweight='bold')
...
def pairdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, length_scaling=True, scale_delim_col="red", title_length=float("Inf")):
    """
    ...
    for idx in range(len(sequences)-1):
        ...
        for jdx in range(idx+1, len(sequences)):
            ...
            # format axes
            P.xlabel(unicode_name(shorten_name(name_one, max_len=title_length)) + " [%s]" % aa_bp_unit, fontsize=label_size, fontweight='bold', labelpad=4)
            P.ylabel(unicode_name(shorten_name(name_two, max_len=title_length)) + " [%s]" % aa_bp_unit, fontsize=label_size, fontweight='bold', labelpad=4)
            P.tick_params(axis='both', which='major', labelsize=label_size*.9)
        ...
        for jdx in range(idx, len(sequences)):
            ...
            for kdx in range(len(counters)):
                ...
                if x_label_bool: # x title and labels on top or bottom
                    P.xlabel(unicode_name(shorten_name(n1, max_len=title_length)), fontsize=label_size, rotation=x_label_rotation, verticalalignment=xvalign, horizontalalignment=xhalign, fontweight='bold', labelpad=8) # axis naming
                ...
                if fig_pos % ncols == 1 or (ncols == 1 and nrows == 1): # y title and labels in 1st column
                    P.ylabel(unicode_name(shorten_name(n2, max_len=title_length)), fontsize=label_size, rotation=y_label_rotation, verticalalignment=yvalign, horizontalalignment=yhalign, fontweight='bold', labelpad=8)
...
def main(seq_list, wordsize, modes=[0, 1, 2], prefix=None, plot_size=10, label_size=10, filetype="png", type_nuc=True, convert_wobbles=False, substitution_count=0, rc_option=True, alphabetic_sorting=False, gff=None, multi=True, ncols=1, nrows=1, lcs_shading=True, lcs_shading_num=5, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, gff_color_config_file="", input_user_matrix_file="", user_matrix_print=False, length_scaling=True, title_length=50, spacing=0.04, verbose=False):
