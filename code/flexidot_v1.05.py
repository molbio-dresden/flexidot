    ...
    for seq_query, seq_target, diag_dict, counter_pos in data_list:
        ...
        for idx in range(len(str(seq_query))-wordsize+1):
            ...
            if kmer.count(any_residue)*100./wordsize <= max_N_percentage:
                ...
                result_matches = regex.finditer(kmer_string, seq_target, overlapped=True, concurrent=True)

                # investigate all hits
                last_motif_start = 0
                for result in result_matches:
                    # skip hits only differing in length of TSD region 1
                    if result.start() > last_motif_start:
                        counter[counter_pos] += 1
                        last_motif_start += result.start()

                        kmer2 = seq_target[result.start():result.end()]

                        # skip excessive N/X stretches (big black areas)
                        if kmer2.count(any_residue)*100./wordsize <= max_N_percentage:
                            diag = idx-(result.start())
