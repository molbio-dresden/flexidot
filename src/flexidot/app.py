...
def read_matrix(
    matrix_file_name, delim="\t", symmetric=True, recursion=False, verbose=False
):
...
def main(
    ...
    if len(input_gff_files) != 0 and input_gff_files != None:
        ...
        if gff_color_config_file not in ["", None]:
            text = (
                "Please provide GFF annotation files to use configuration file",
                gff_color_config_file,
            )
            logprint(text, start=False, printing=True)
...
main(
    input_fasta,
    wordsize,
    modes=plotting_modes,
    prefix=output_file_prefix,
    plot_size=plot_size,
    label_size=label_size,
    filetype=filetype,
    type_nuc=type_nuc,
    convert_wobbles=wobble_conversion,
    substitution_count=substitution_count,
    rc_option=rc_option,
    alphabetic_sorting=alphabetic_sorting,
    only_vs_first_seq=only_vs_first_seq,
    gff=input_gff_files,
    multi=collage_output,
    ncols=m_col,
    nrows=n_row,
    lcs_shading=lcs_shading,
    lcs_shading_num=lcs_shading_num,
    ...
    lcs_shading_interval_len=lcs_shading_interval_len,
    lcs_shading_ori=lcs_shading_ori,
    gff_color_config_file=gff_color_config_file,
    input_user_matrix_file=input_user_matrix_file,
    user_matrix_print=user_matrix_print,
    length_scaling=length_scaling,
    title_length=title_length,
